...
class PredictionService:
    ...
    async def predict(self, request: PredictionRequest) -> PredictionResponse:
        ...
        return PredictionResponse(
            ...
            prediction=inference_result.get("prediction", saved_record["disease"]),
            confidence=saved_record["confidence"],
            confidence_formatted=inference_result.get("confidence_formatted", f"{saved_record['confidence'] * 100:.1f}%"),
            model=inference_result.get("model", "Chest X-ray Model v1.0"),
            disclaimer=inference_result.get("disclaimer", "⚠️ AI-assisted result. This system is not a replacement for professional medical diagnosis."),
